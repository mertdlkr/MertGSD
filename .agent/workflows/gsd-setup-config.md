---
description: Configure project-specific GSD settings (notifications, deploy platform, model preferences)
---

# GSD Setup Config — Project Configuration

Interactive configuration for project-specific GSD settings. Creates `.planning/gsd-config.json`.

## When to Run

- Automatically called by `gsd-new-project` after project initialization
- Manually via `/gsd-setup-config` to update settings
...
```bash
cat .planning/gsd-config.json 2>/dev/null
```
...
- Test notification: `curl -s -d "GSD bildirim testi basarili!" ntfy.sh/${topic}`
...
- "Autonomous execution allowed?" → yes / no (affects gsd-super)
...
Write `.planning/gsd-config.json`:
